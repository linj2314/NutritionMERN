...
export default function InputFood() {
    ...
    function EnterAmt() {
      const [multiplier, updateMultiplier] = useState(100);
      return(
        <>
          <input type="text" placeholder="Enter Quanitity (grams)" onChange={e => {updateMultiplier(e.target.value); tempmult = e.target.value}}></input>
          <div>
          <h2>Selected Item:</h2>
      ...
      <>
        ...
        <div style={{display: "flex"}}>
          ...
          <button style={{marginLeft: "80%", position: "absolute"}} onClick={() => {
              ...
              let tempselected = {
                name: selected.name,
                calories: Math.round((selected.calories  * tempmult/100) * 1000) / 1000,
                protein: Math.round((selected.protein * tempmult/100) * 1000) / 1000,
                carbs: Math.round((selected.carbs  * tempmult/100) * 1000) / 1000,
                fats: Math.round((selected.fats  * tempmult/100) * 1000) / 1000
              };
              temp.push(tempselected);
              setCurrentList(temp);
              navigate("/")
          }}>Submit</button>
